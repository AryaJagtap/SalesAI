"""
Sales Call Analysis - Ultimate Edition (Bug Fix)
Author: AJAX
Features:
- UI: Modern SaaS UI with detailed Breakdown
- Logic: Google Gemini 2.5 Flash (Sentiment, Intent, Entities, Suggestions)
- Fix: Resolved 'AttributeError: transcript' by initializing session state correctly.
"""
...
st.set_page_config(
    page_title="SalesAI Co-Pilot (Ultimate)",
    page_icon="⚡",
    layout="wide",
...
def analyze_with_gemini(text, api_key):
    ...
    try:
        ...
        Analyze this sales conversation snippet. Return ONLY valid JSON with this structure:
        {{
            "sentiment": {{ "label": "Positive/Neutral/Negative", "score": 0.0 to 1.0 }},
            "intent": {{ "label": "Buy/Inquire/Complain/etc", "score": 0.0 to 1.0 }},
            "entities": [ {{ "entity": "Type (Brand/Product/Budget)", "value": "Extracted Value" }} ],
            "suggestion": "The best next question for the sales agent to ask.",
...
if 'transcript' not in st.session_state: st.session_state.transcript = []  # <--- THIS WAS MISSING
...
with st.sidebar:
    ...
    user_key_input = st.text_input("Gemini API Key", type="password", placeholder="Paste key here...")
    
    if user_key_input:
        st.session_state.active_key = user_key_input
        st.success("✅ Key Detected & Ready")
    else:
...
if st.session_state.page == 'Live':
    st.markdown("## 🎙️ Live Sales Co-Pilot")
    st.markdown("Record the customer. Gemini will analyze context and suggest the best sales tactic.")
        ...
        with st.spinner("Listening..."):
            ...
            if text:
                ...
                with st.spinner("🧠 Gemini is analyzing..."):
                    analysis = analyze_with_gemini(text, current_key)
